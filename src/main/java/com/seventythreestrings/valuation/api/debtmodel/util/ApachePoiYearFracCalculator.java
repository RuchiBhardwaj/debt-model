/*
 * As this utility is internal to the Apache Poi package 'org.apache.poi.ss.formula.atp', we are copying this to our package
 * in order to make use of the YearFracCalculator. The public APIs for this are related to manipulating excel cells.

 * ref: https://github.com/apache/poi/blob/b6aee1ef6d3e92a28ffd4b5c03e677b63b43747f/poi/src/main/java/org/apache/poi/ss/formula/atp/YearFracCalculator.java
 */
 ...
 */
final class ApachePoiYearFracCalculator {
    ...
    private ApachePoiYearFracCalculator() {
        // no instances of this class
    }


    public static double calculate(double pStartDateVal, double pEndDateVal, int basis) throws IllegalStateException {

        if (basis < 0 || basis >= 5) {
            // if basis is invalid the result is #NUM!
            // throw new EvaluationException(ErrorEval.NUM_ERROR);
            // TODO:
            throw new IllegalStateException("basis should be between 0 and 4");
        }
        ...
        switch (basis) {
            ...
            case 1: return basis1Custom(startDateVal, endDateVal);
    ...
    private static double basis1Custom(int startDateVal, int endDateVal) {
        assert(startDateVal <= endDateVal);
        SimpleDate startDate = createDate(startDateVal);
        SimpleDate endDate = createDate(endDateVal);

        // check if it spans across years
        // if either of the years is a leap year, the actual days vary
        if(startDate.year == endDate.year) {
            return basis1(startDateVal, endDateVal);
        } else if (isLeapYear(startDate.year) || isLeapYear(endDate.year)) {
            double basis = 0;
            int intermediateDateVal = isLeapYear(startDate.year) ? getLastDateForYear(startDate.year) : getFirstDateForYear(endDate.year);
            int intermediateEndDateVal = getLastDateForYear(startDate.year);
            int intermediateStartDateVal = getFirstDateForYear(endDate.year);
            if (startDateVal != intermediateDateVal) {
                double basis1 = basis1(startDateVal, intermediateDateVal);
                basis += basis1;
            }

            if (intermediateDateVal != endDateVal) {
                double basis2 = basis1(intermediateDateVal, endDateVal);
                basis += basis2;
            }
    ...
    private static int getFirstDateForYear(int year) {
        Calendar cal = Calendar.getInstance(TimeZone.getTimeZone("UTC"));
        cal.set(year, Calendar.JANUARY, 1);

        return (int) Math.floor(ApachePoiDateUtil.getExcelDate(cal));
    }

    private static int getLastDateForYear(int year) {
        Calendar cal = Calendar.getInstance(TimeZone.getTimeZone("UTC"));
    ...
    private static SimpleDate createDate(int dayCount) {
        ...
        Calendar cal = Calendar.getInstance(TimeZone.getTimeZone("UTC"));
        ApachePoiDateUtil.setCalendar(cal, dayCount);
